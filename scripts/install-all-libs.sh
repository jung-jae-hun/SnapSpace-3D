...
pnpm add -Dw typescript @types/node eslint prettier
...
pnpm add -F @snapspace/api @prisma/client ioredis bullmq minio
pnpm add -F @snapspace/api -D @nestjs/cli prisma ts-node ts-node-dev
